...
// clusterOptions 组装集群连接 Option 列表。
...
// 均未设置时整体跳过，保证本地/CI 离线测试套件不受影响。
func TestClusterIntegration(t *testing.T) {
	addrs := clusterAddrs()
	if len(addrs) == 0 {
		t.Skip("REDIS_CLUSTER_ADDRS 未设置，跳过集群集成测试")
	}

	pwd := clusterPassword()
	opts := clusterOptions(addrs, pwd)
	...
	t.Run("前缀在集群重定向下正确", func(t *testing.T) {
		...
		plain := redis.New(opts...)
		defer func() { _ = plain.GracefulClose(context.Background()) }()

		prefixed := redis.New(append(opts, redis.WithPrefix("itest"))...)
	...
	t.Run("LoadFunction 全主节点加载", func(t *testing.T) {
		...
		assert.GreaterOrEqual(t, masterCount, 1, "应至少遍历到一个主节点")
	})
